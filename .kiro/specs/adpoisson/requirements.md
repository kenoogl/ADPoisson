...
- `config.yaml` の `postprocess.commands`（文字列配列）を指定した場合、`run_exp` は solver 実行後に
  各コマンドを順次実行する
  - 例: `julia --project scripts/history_stats.jl --input-dir results/<exp>`
  - 後処理コマンドが失敗しても `run_exp` は警告のみを出して継続し、実験本体は成功扱いとする
