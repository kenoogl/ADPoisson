...
- **方針**: レベルごとの作業配列を初回に確保し、以降は **配列全域（境界/ゴースト領域を含む）を `fill!`** で初期化して再利用する（`zeros/similar` を反復内で呼ばない）。
...
- `postprocess.commands`（文字列配列）をサポートし、solver 実行後に順次実行する。
  - 例: `scripts/history_stats.jl --input-dir results/<exp>` を command として設定可能。
